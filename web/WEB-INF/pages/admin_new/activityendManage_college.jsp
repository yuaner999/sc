...
<head>
    ...
    <script type="text/javascript">
        ...
        var loadUrl = "/check/loadCheckMajorNum.form";//注：此处为新增
        var sqlStr = "";//注：模糊查询
        var jsonPara;//参数
        var collegeName=decodeURI(decodeURI(GetQueryString("stuCollegeName")));
        ...
        function before_reload(){
            ...
            if(collegeId==""||collegeId==null||collegeId=='null'){
                status='sc';
                jsonPara={rows:rows,page:page,collegeName:collegeName,status:status,loadstatus:loadstatus};
            }else{
                status='co';
                        ...
                        for (var i = 0; i < data.rows.length; i++) {
                            ...
                            var tr = '<tr id="tr'+(i+1)+'">'+
                                    '<td>'+ '<input name="test" class="checkes" type="checkbox" id="submint'+i+'" style="width:18px;height: 20px;" />'+'</td>'+
                                        //第二处修改：按照数据库列名进行拼穿
                                    '<td style="color: #1990FE;cursor:pointer;" onclick="go(\''+row.stuMajorName+'\')">'+row.stuMajorName+'</td>'+
                                    '<td>'+row.totalNum+'</td>'+
                                    '<td >' +
...
//                                    '<td class="regimentAuditStatus"  style="display:none">'+row.regimentAuditStatus+'</td>'+
//                                    '<td class="regimentAuditStatusDate"  style="display:none">'+row.regimentAuditStatusDate+'</td>'+
//                                    '<td class="collegeAuditStatus"  >'+row.collegeAuditStatus+'</td>'+
//                                    '<td class="collegeAuditStatusDate"  >'+row.collegeAuditStatusDate+'</td>'+
...
//            console.log(collegeName);
            ...
            layer.confirm("确认此操作吗？！！！",function(result){
                if (result){
                $.ajax({
                    url: "/check/CollegeAuditApply.form",
                    type: "post",
                    data: {stuMajorName: stuMajorName.substring(0, stuMajorName.length - 1), type: type,collegeName: collegeName,status:status},
                    dataType: "json",
                    success: function (data) {
                        if (data) {
            ...
            layer.confirm("确认此操作吗？！！！",function(result){
                if (result){
                    $.ajax({
                        url: "/check/CollegeAuditApply.form",
                        type: "post",
                        data: {stuMajorName: stuMajorName, type: type,collegeName: collegeName,status:status},
                        dataType: "json",
                        success: function (data) {
                            if (data) {
                                if(parseInt(data.msg)===0){
        ...
        function  go(val){
            ...
            window.open('<%=request.getContextPath()%>/views/admin_new/activityendManage_class.form?stuClassName='+val+'&loadstatus='+encodeURI(encodeURI(loadstatus)),'_parent');
        ...
        function reloadsearch(val){
            loadstatus=val;
            jsonPara={rows:rows,page:'1',collegeName:collegeName,status:status,loadstatus:loadstatus};
            reload();
        }
    </script>
    <%--判断是否是登录状态--%>
