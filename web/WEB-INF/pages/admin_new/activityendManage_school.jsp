...
<head>
    ...
    <script type="text/javascript">
        ...
        var loadUrl = "/check/loadCheckNum.form";//注：此处为新增
        var sqlStr = "";//注：模糊查询
        var jsonPara;//参数
        var loadstatus='待审核';
        ...
        function before_reload(){
            ...
            if(sqlStr==""||sqlStr==null){
                jsonPara={rows:rows,page:page,loadstatus:loadstatus};
            }else{
                jsonPara={rows:rows,page:page,sqlStr:sqlStr,loadstatus:loadstatus};
            }
                        ...
                        for (var i = 0; i < data.rows.length; i++) {
                            ...
                            var tr = '<tr id="tr'+(i+1)+'">'+
                                    '<td>'+ '<input name="test" class="checkes" type="checkbox" id="submint'+i+'" style="width:18px;height: 20px;" />'+'</td>'+
                                        //第二处修改：按照数据库列名进行拼穿
                                    '<td style="color: #1990FE;cursor:pointer;" onclick="go(\''+row.stuCollageName+'\')">'+row.stuCollageName+'</td>'+
                                    '<td>'+row.totalNum+'</td>'+
                                    '<td >' +
...
//                                    '<td class="activitypoint" title="'+row.activitypoint+'" >'+row.activitypoint+'</td>'+
...
//                                    '<td class="schoolAuditStausDate"  >'+row.schoolAuditStausDate+'</td>'+
                                    '</tr>';
                            $("tbody").append(tr);
 ...
 //                       rowClick();//绑定行点击事件
        ...
        function auditsAction(type){
            ...
            $('tbody input:checkbox:checked').each(function() {
                ...
                stuCollageName+=row.stuCollageName+"|";
                totalNum+=parseInt(row.totalNum);
            });
//            console.log(totalNum);
//            console.log(stuCollageName);
            if(stuCollageName==""){
                layer.msg("请选择一行数据");
                return;
            }
            ...
            layer.confirm("确认此操作吗？！！！",function(result){
                if (result){
                $.ajax({
                    url:"/check/SchoolAuditApply.form",
                    type: "post",
                    data:{stuCollageName:stuCollageName.substring(0,stuCollageName.length-1),type:type},
                    dataType: "json",
                    success: function (data) {
                        if(data){
            ...
            layer.confirm("确认此操作吗？！！！",function(result){
                if (result){
                    $.ajax({
                        url:"/check/SchoolAuditApply.form",
                        type: "post",
                        data:{stuCollageName:stuCollageName,type:type},
                        dataType: "json",
                        success: function (data) {
                            if(data){
                                if(parseInt(data.msg)===0){
        ...
        function  go(val){
            ...
            window.open('<%=request.getContextPath()%>/views/admin_new/activityendManage_college.form?stuCollegeName='+val+'&loadstatus='+encodeURI(encodeURI(loadstatus)),'_parent');
        ...
        function reloadsearch(val){
            loadstatus=val;
            jsonPara={rows:rows,page:'1',sqlStr:sqlStr,loadstatus:loadstatus};
            reload();
        }
    </script>
    <%--判断是否是登录状态--%>
