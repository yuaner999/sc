...
<head>
    ...
    <script type="text/javascript">
        ...
        var loadUrl = "/jsons/loadSupplementSchoolmybatis.form";//注：此处为新增
        var sqlStr = "";//注：模糊查询
        var jsonPara;//参数
        var applyId="";
        var statusLevel="";
                    ...
                    if (data != null && data.rows != null && data.rows.length > 0) {
                        for (var i = 0; i < data.rows.length; i++) {
                            ...
                            if(row['supClass']){
                                switch(row['supClass']) {
                                    case '1':
                                        row['supClasses'] ='思想政治教育类'; break;
                                    case '2':
                                        row['supClasses'] ='能力素质拓展类'; break;
                                    case '3':
                                        row['supClasses'] ='学术科技与创新创业类'; break;
                                    case '4':
                                        row['supClasses'] ='社会实践与志愿服务类'; break;
                                    case '5':
                                        row['supClasses'] ='社会工作与技能培训类'; break;
                                    case '6':
                                        row['supClasses'] ='综合奖励及其他类'; break;
                                    default:
                                        row['supClasses'] ='';
                                }
                            }
                            ...
                            var tr = '<tr id="tr'+(i+1)+'" onclick=" SearchDetail(this)">'+
                                    ...
                                    '<td class="X" title="'+row.supActivityTitle +'">'+ttt+'</td>'+
                                    '<td class="X" title="'+row.supClasses +'">'+row.supClasses+'</td>'+
                                    ...
                                    '<td>' +
                                    '<li class="function_new function_auditPass" style="width: 70px;float:left;cursor: pointer" onclick="stopBubble(this),auditsAction(this,\'已通过\')"><span>通过</span></li>' +
                                    '<li class="function_edit function_auditNotPass" style="width: 70px;float: right;cursor: pointer" onclick="stopBubble(this),auditsAction(this,\'未通过\')"><span>不通过</span></li>' +
                                    '</td>'+
...
//                        console.log(val)
        ...
        //班级审核通过
        function auditsAction_class(val){
            var type='已通过';
            var row=$(val).parents("td").parents("tr").data();
            //增加能力复选框验证
...
//                        $("#score").val('');
                        $("#btn_sub").click(function(){
                            ...
                            if(1){
                                $.ajax({
                                    ...
                                    type: "post",
                                    data:{applyId:applyId,Type:type,score:score},
                                    dataType: "json",
                                    success: function (data) {
                                        if(data){
                                            layer.msg(data.msg);
        ...
        //学院审核通过
        function auditsAction_college(val){
            var type='已通过';
            var row=$(val).parents("td").parents("tr").data();
            //增加能力复选框验证
//            var qxs=$(".checkes:checked");
...
//            });
            ...
            if(regimentAuditStatuses.indexOf("未通过")>=0||regimentAuditStatuses.indexOf("修改")>=0){
                layer.msg("操作失败,班级审核结果为'未通过'或'修改'");
                return;
            }
            ...
            layer.confirm('确认此操作吗?!', function(result) {
                ...
                if (result){
                    //验证重复审核
                    if (collegeAuditStatuses.indexOf("未通过")>=0||collegeAuditStatuses.indexOf("已通过")>=0||collegeAuditStatuses.indexOf("修改")>=0) {
                        layer.msg("您选中的项目已审核过，不能重复审核");
                        return;
                    } else {
                        $("#tb2").show();
...
//                        $("#score").val('');
        ...
        //学校审核通过
        function auditsAction_school(val){
...
//            });
            ...
            if(collegeAuditStatuses.indexOf("未通过")>=0||collegeAuditStatuses.indexOf("修改")>=0){
                layer.msg("操作失败,学院审核结果为'未通过'或'修改'");
                return;
            }
...
//                    $("#score").val('');
                    $("#btn_sub").click(function(){
                        var score=  $("#score").val();
                        $.ajax({
                            url:"/apply/SchoolAuditSupplementScore.form",
                            type: "post",
                            data:{applyId:applyId,Type:type,score:score},
                            dataType: "json",
                            success: function (data) {
...
//            console.log(supPhoto);
        ...
        //班级审核不通过
        function auditsAction_no_class(val){
            var type='未通过';
            var row=$(val).parents("td").parents("tr").data();
//            var qxs=$(".checkes:checked");
            applyId = "";
...
//                    var row = $(this).parent().parent().data();
                    applyId = row.id ;
                    regimentAuditStatuses=row.regimentAuditStatus==null?'':row.regimentAuditStatus;
                    collegeAuditStatuses=row.collegeAuditStatus;
                    schoolAuditStauses=row.schoolAuditStaus;
//                });
                if(applyId==""){
...
//            }
        ...
        //审核不通过
        function auditsAction_no_college(val){
            var type='未通过';
            var row=$(val).parents("td").parents("tr").data();
            applyId = "";
...
//                    var row = $(this).parent().parent().data();
            applyId = row.id;
            regimentAuditStatuses=row.regimentAuditStatus;
            collegeAuditStatus = row.collegeAuditStatus;
//                });
                if(regimentAuditStatuses.indexOf("未通过")>=0||regimentAuditStatuses.indexOf("修改")>=0){
            ...
            if (collegeAuditStatus.indexOf('未通过') >= 0 || collegeAuditStatus.indexOf('已通过') >= 0||collegeAuditStatus.indexOf('修改') >= 0) {
                layer.msg("您选中的项目已经审核过，不能重复审核");
                return;
            }
            $("#dlg2").show();
            $("#co_score").hide();
            ...
            $("#btn_sub").click(function () {
                ...
                layer.confirm('确认此操作吗?!', function (result) {
                    if (result) {
                        $.ajax({
                            url: "/jsons/CollegeAuditOneSupplement.form",
                            type: "post",
                            data: {applyId: applyId,reason:reason, Type: type},
                            dataType: "json",
                            success: function (data) {
        ...
        //学校审核不通过
        function auditsAction_no_school(val){
            var type='未通过';
            var row=$(val).parents("td").parents("tr").data();
//            var qxs=$(".checkes:checked");
            applyId = "";
...
//                if(value=='已通过') {
//                    auditsAction_class(value);
//                    return;
//                }
//                if(value=='修改'||value=='未通过') {
//                    auditsAction_no_class(value);
//                    return;
//                }
//            }
//            if(statusLevel=='college'){
//                if(value=='已通过'){
//                    auditsAction_college(value);
//                    return;
//                }
//                if(value=='修改'||value=='未通过'){
//                    auditsAction_no_college(value);
//                    return;
//                }
//            }
//            if(statusLevel=='school') {
//                if (value == '已通过') {
//                    auditsAction_school(value);
//                    return;
//                }
//                if (value == '修改' || value == '未通过') {
//                    auditsAction_no_school(value);
//                    return;
//                }
//            }
//        }

        function auditsAction(value,type){
            if(statusLevel==''){
                layer.msg("请先选取审核级别");
                return;
            }
...
//                        $("#score").val('');
        ...
        //学院审核通过
        function batchpass_college(type){
            //增加能力复选框验证
            var qxs=$(".checkes:checked");
            if(qxs.length<1){
                layer.msg("请选择",{offset:['30%'] });
...
//                        $("#score").val('');
        ...
        //学校审核通过
        function batchpass_school(type){
...
//                    $("#score").val('');
        ...
        //班级审核不通过
        function batchrefuse_no_class(type){
            var qxs=$(".checkes:checked");
            applyId = "";
            var regimentAuditStatuses="";
            var collegeAuditStatuses="";
        ...
        //审核不通过
        function batchrefuse_no_college(type){
            applyId = "";
            var collegeAuditStatus = '';
            var regimentAuditStatuses='';
            //增加能力复选框验证
            ...
            if(qxs.length<1){
                ...
                layer.confirm('确认此操作吗?!', function (result) {
                    if (result) {
                        //验证重复审核
                        $.ajax({
                            url: "/apply/CollegeAuditSupplement.form",
                            type: "post",
                            data: {applyId: applyId.substring(0, applyId.length - 1), Type: type},
                            dataType: "json",
                            success: function (data) {
        ...
        //学校审核不通过
        function batchrefuse_no_school(type){
            var qxs=$(".checkes:checked");
            applyId = "";
            var regimentAuditStatuses="";
            var collegeAuditStatuses="";
        ...
        function batchpass(){
            ...
            if(statusLevel=='college'){
                batchpass_college("已通过");
                return;
            }
            if(statusLevel=='school'){
                batchpass_school("已通过");
                return;
            }
        }
        //批量审核不通过
        function batchrefuse(){
            ...
            if(statusLevel=='class'){
                batchrefuse_no_class("未通过");
                return;
            }
            if(statusLevel=='college'){
                batchrefuse_no_college("未通过");
                return;
            }
            if(statusLevel=='school'){
                batchrefuse_no_school("未通过");
                return;
            }
        }
    </script>
...
<div class="stuMsgManage">
    ...
    <div class="function">
        <ul>
            ...
            <%--<li id="yes" class="function_new function_auditPass" onclick="audit('已通过')" style="display: none"><span>通过</span></li>--%>
            <%--<li id="edit" class="function_new function_auditNotPass" onclick="audit('修改')" style="display: none"><span>修改</span></li>--%>
            <%--<li id="no" class="function_edit function_auditNotPass" onclick="audit('未通过')" style="display: none"><span>未通过</span></li>--%>
        </ul>
    </div>
    <form id="Form1" >
...
0px 0px 12px;"/>创业能力
            ...
            <li>
                <span>参与时间&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;</span>
                <input id="takeDates" name="takeDates" class="select"  style="margin-left: 21px;" readonly="true"/>
            </li>
            <li>
                <span>填写时间&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;</span>
                <input id="supDates" name="supDates" class="select"  style="margin-left: 21px;" readonly="true"/>
            </li>
            <li>
                <span>可得学分&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;</span>
                <!--表单验证例子如下 -->
        ...
        <ul class="btnbox">
            ...
            <li><span class="sure iconConcel">取消</span></li>
        </ul>
    </div>
</form>
