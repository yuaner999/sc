...
//                                    '<td class="X" title="'+row.schoolAuditStaus +'">'+row.schoolAuditStaus+'</td>'+
                                    '<td >' +
                                    '<li class="function_new function_auditPass" style="width: 75px;float:left;cursor: pointer" onclick="stopBubble(this),auditsAction(this)"><span>通过</span></li>' +
                                    '<li class="function_edit function_auditNotPass" style="width: 75px;float: right;cursor: pointer" onclick="stopBubble(this),auditsAction_no(this)"><span>不通过</span></li>' +
                                    '</td>'+
                                        // '<td><a  href="javascript:void(0);" onclick="SearchDetail(this)">查看详情</a></td>'+
                                    '</tr>';
                            $("tbody").append(tr);
                            $("tbody").find("tr:last").data(row);
                            var box=row.supCredit;
                        }
                        totalNum=data.total;
...
//            });
            ...
            if(regimentAuditStatuses.indexOf("未通过")>=0||regimentAuditStatuses.indexOf("修改")>=0){
                layer.msg("操作失败,该条数据班级审核结果为'未通过'");
                return;
            }
...
//            }
        ...
        /*function modify_zy(){
            $('input:checkbox:checked').each(function () {
                var rowM = $(this).parent().parent().data();
            });
            if(!rowM) {
                layer.alert("请先选择一条数据");
                return
...
<div class="stuMsgManage">
    ...
    <div class="function">
        <ul>
            <%--<li class="function_edit" onclick="modify_zy()"><span>修改</span></li>--%>
            ...
            <%--&lt;%&ndash;<li class="function_new function_auditNotPass" onclick="auditsAction_no('修改')"><span>修改</span></li>&ndash;%&gt;--%>
            <%--<li class="function_auditNotPass" onclick="auditsAction_no('未通过')"><span>未通过</span></li>--%>
        </ul>
    </div>
    <!--综合查询部分-->
    <form id="Form1" >
...
0px 0px 12px;"/>创业能力
            ...
            <li>
                <span>学院审核时间&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;</span>
                <input id="collegeAuditStatusDate" name="collegeAuditStatusDate" class="select"  readonly="true"/>
            </li>
            <li>
                <span>参与时间&nbsp;&nbsp;:&nbsp;&nbsp;&nbsp;&nbsp;</span>
        ...
        <ul class="btnbox">
            <li  onclick="Saves()"><span class="sure">确定</span></li>
...
<div id="dlg2" class="new" style="min-width: 400px;height: 200px;left: 450px">
    ...
    <ul>
        ...
        <li id="co_reason" style="display: none">
